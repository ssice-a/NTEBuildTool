...
- **Standard Runtime Template Contract**: The narrow interface a template must expose to the generator. Each setup group is a user-authored toggle item with a label, optional hotkey, default state, and one or more material slots. Visible-state variables are named `NTE_Toggle_XX_toggle_group_N_Visible`, input marker variables are named `NTE_Toggle_Input_N_...`, WBP buttons/text labels are named `NTE_Toggle_Button_XX_toggle_group_N` and `NTE_Toggle_Button_XX_toggle_group_N_Label`, drag widgets are named `NTE_Toggle_TitleBarButton` and `NTE_Toggle_WindowPanel`, the generated Widget Blueprint owns drag-to-move behavior in its own Tick graph, polls `NTE_Toggle_TitleBarButton.IsPressed()`, reads viewport mouse position from `Self` world context, template group capacity must be at least the setup group count, unused template groups are disabled and hidden in the generated Widget Blueprint, UI key placeholders use the standard `Slash` plus modifier-key pins, repeated `ShowMaterialSection` nodes for the same group are assigned to configured slots cyclically, and non-ASCII labels trigger an embedded Unicode font in the generated Widget Blueprint.
...
- **Toggle Item**: One user-authored entry inside a Toggle Setup. It must have a non-empty UI label and at least one material slot. Its hotkey is optional; if present, it must not duplicate another toggle item or the UI show/hide chord.
...
- **Mesh Mod Workspace**: The user-facing editor workflow centered on one selected mesh. It presents the selected mesh, material slots, texture replacement, Toggle Items, generated runtime assets, and Package Plan candidates without trying to prove whether the mesh path is the intended original game path. It is an orchestration interface over the material module, toggle-runtime module, and package pipeline module, not a place for their implementations.
