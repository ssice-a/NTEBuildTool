...
FString DeriveRuntimeRootPath(const FNteCharacterModSpec& Spec)
{
	...
	// UI and SaveGame assets are shared by every runtime-action host. Prefer the
	// main host when it exists so unrelated attached-mesh/Kawaii AnimBPs cannot
	// relocate the shared assets merely by appearing first in the spec.
	for (const FNteCharacterRuntimeActionSpec& Action : Spec.RuntimeActions)
	{
		if (IsMainMeshId(Action.HostMeshId))
		{
			continue;
		}

		const FNteCharacterAttachedMeshSpec* HostMesh = FindAttachedMeshById(Spec, Action.HostMeshId);
		if (HostMesh == nullptr)
		{
			continue;
		}

		const FString RuntimeAnimDirectory = HostMesh->RuntimeAnimBlueprintPath.Contains(TEXT("/mod/"), ESearchCase::IgnoreCase)
			? GetAssetDirectory(HostMesh->RuntimeAnimBlueprintPath)
			: FString();
		...
		const FString AttachedAnimDirectory = HostMesh->AnimBlueprintPath.Contains(TEXT("/mod/"), ESearchCase::IgnoreCase)
			? GetAssetDirectory(HostMesh->AnimBlueprintPath)
			: FString();
