...
bool IsBoneInRequestedChain(const USkeletalMesh& SkeletalMesh, const FName CandidateBone, const FName ChainRootBone)
{
	if (ChainRootBone == NAME_None)
	{
		return true;
	}

	const FReferenceSkeleton& ReferenceSkeleton = SkeletalMesh.GetRefSkeleton();
	const int32 RootIndex = ReferenceSkeleton.FindBoneIndex(ChainRootBone);
	int32 CurrentIndex = ReferenceSkeleton.FindBoneIndex(CandidateBone);
...
bool IsBodyInRequestedChain(const USkeletalMesh& SkeletalMesh, const FJsonObject& BodyJson, const FName ChainRootBone)
{
	TSharedPtr<FJsonObject> Properties;
	return TryGetObject(BodyJson, TEXT("Properties"), Properties)
		&& IsBoneInRequestedChain(SkeletalMesh, GetOptionalName(*Properties, TEXT("BoneName")), ChainRootBone);
}

bool IsConstraintInRequestedChain(const USkeletalMesh& SkeletalMesh, const FJsonObject& ConstraintJson, const FName ChainRootBone)
{
	TSharedPtr<FJsonObject> Properties;
	TSharedPtr<FJsonObject> Instance;
	return TryGetObject(ConstraintJson, TEXT("Properties"), Properties)
		&& TryGetObject(*Properties, TEXT("DefaultInstance"), Instance)
		&& IsBoneInRequestedChain(SkeletalMesh, GetOptionalName(*Instance, TEXT("ConstraintBone1")), ChainRootBone)
		&& IsBoneInRequestedChain(SkeletalMesh, GetOptionalName(*Instance, TEXT("ConstraintBone2")), ChainRootBone);
}
...
UPhysicsAsset* CreatePhysicsAsset(USkeletalMesh& SkeletalMesh, const FFModelPhysicsAssetAnalysis& Analysis, const FFModelPhysicsAssetImportOptions& Options, FString& OutError)
{
	...
	{
		if (!Body.IsValid() || !IsBodyInRequestedChain(SkeletalMesh, *Body, Options.ChainRootBone))
		{
			continue;
		}
		if (!BuildBodyFromJson(*PhysicsAsset, SkeletalMesh, *Body, OutError))
		{
			return nullptr;
		}
	}
	...
	{
		if (!Constraint.IsValid() || !IsConstraintInRequestedChain(SkeletalMesh, *Constraint, Options.ChainRootBone))
		{
			continue;
		}
		if (!BuildConstraintFromJson(*PhysicsAsset, SkeletalMesh, *Constraint, OutError))
		{
			return nullptr;
		}
	}

	if (Options.ChainRootBone == NAME_None)
	{
	...
	{
		OutError = Options.ChainRootBone == NAME_None
			? TEXT("The source PhysicsAsset contains no supported bodies.")
			: FString::Printf(TEXT("The selected chain root '%s' did not match any source PhysicsAsset body on the selected SkeletalMesh."), *Options.ChainRootBone.ToString());
		return nullptr;
	}
...
{
	return ImportFromJsonFile(SkeletalMesh, JsonFilePath, FFModelPhysicsAssetImportOptions(), OutSummary, OutError);
}

UPhysicsAsset* FFModelPhysicsAssetImporter::ImportFromJsonFile(
	USkeletalMesh& SkeletalMesh,
	const FString& JsonFilePath,
	const FFModelPhysicsAssetImportOptions& Options,
	FFModelPhysicsAssetImportSummary& OutSummary,
	FString& OutError)
{
	...
	UPhysicsAsset* PhysicsAsset = CreatePhysicsAsset(SkeletalMesh, Analysis, Options, OutError);
	if (!PhysicsAsset)
	{
		return nullptr;
	}
	...
	OutSummary.DisabledCollisionPairCount = Options.ChainRootBone == NAME_None ? Analysis.DisabledCollisionPairCount : 0;
	return PhysicsAsset;
}
}
