...
void RunCreateModMaterialInstanceFromGameLibrary(const NTEBuildTool::Material::FNteMaterialInstanceOptions& InitialOptions)
{
	NTEBuildTool::Material::FNteMaterialInstanceOptions Options;
	TSharedPtr<FJsonObject> SourceTextureOverrides;
	if (!NTEBuildTool::Material::ShowFModelMaterialLibraryRecipeDialog(InitialOptions, Options, SourceTextureOverrides))
	{
		return;
	}
	...
	const FScopedTransaction Transaction(LOCTEXT("CreateModMaterialInstanceFromLibraryTransaction", "Create Mod Material Instance From Game Material Library"));
	...
	NTEBuildTool::Editor::ShowSuccessNotification(FText::Format(
		LOCTEXT("CreatedModMaterialInstanceFromLibrary", "Created/updated {0}. Source texture groups: {1}. Report: {2}"),
...
void RunCharacterMaterialOperationFromGameLibrary(
	USkeletalMesh* SelectedMesh,
	const NTEBuildTool::Workspace::FNteMeshModWorkspaceResult& WorkspaceResult)
{
	NTEBuildTool::Material::FNteMaterialInstanceOptions Options;
	TSharedPtr<FJsonObject> SourceTextureOverrides;
	if (!NTEBuildTool::Material::ShowFModelMaterialLibraryRecipeDialog(
	...
	Operation.SourceMaterialJson = Options.SourceMaterialJson;
...
void FNTEBuildToolModule::RegisterMenus()
{
	...
	Section.AddEntry(FToolMenuEntry::InitMenuEntry(
		TEXT("NTEBuildTool_ImportPhysicsAssetFromGameLibrary"),
		LOCTEXT("ImportFModelPhysicsAssetLabel", "Import PhysicsAsset From Game Library"),
		LOCTEXT("ImportFModelPhysicsAssetTooltip", "Browse Player and NPC PhysicsAsset exports, then import the full preset or a selected matching bone chain."),
		FSlateIcon(),
		FUIAction(FExecuteAction::CreateRaw(this, &FNTEBuildToolModule::ImportPhysicsAssetFromGameLibrary))));
	Section.AddEntry(FToolMenuEntry::InitMenuEntry(
		TEXT("NTEBuildTool_CreateModMaterialInstanceFromGameLibrary"),
		LOCTEXT("CreateModMaterialInstanceFromGameLibraryLabel", "Create Material Instance From Game Material Library"),
		LOCTEXT("CreateModMaterialInstanceFromGameLibraryTooltip", "Browse Player and NPC material instances exported by FModel, then create a mod MaterialInstanceConstant with selected texture overrides."),
		FSlateIcon(),
		FUIAction(FExecuteAction::CreateRaw(this, &FNTEBuildToolModule::CreateModMaterialInstanceFromGameLibrary))));
	Section.AddEntry(FToolMenuEntry::InitMenuEntry(
...
void FNTEBuildToolModule::OpenCharacterModWorkspace()
{
	...
	case NTEBuildTool::Workspace::ENteMeshModWorkspaceAction::ApplyMaterialOperation:
		RunCharacterMaterialOperationFromGameLibrary(SelectedSkeletalMesh, WorkspaceResult);
		break;
...
void FNTEBuildToolModule::ImportPhysicsAssetFromGameLibrary()
{
	USkeletalMesh* SelectedSkeletalMesh = NTEBuildTool::Editor::GetSingleSelectedSkeletalMesh();
	...
	NTEBuildTool::Physics::FNteFModelPhysicsAssetLibrarySelection Selection;
	if (!NTEBuildTool::Physics::ShowFModelPhysicsAssetLibraryDialog(*SelectedSkeletalMesh, Selection))
	{
		return;
	}
	...
	NTEBuildTool::FFModelPhysicsAssetImportOptions ImportOptions;
	ImportOptions.ChainRootBone = Selection.ChainRootBone;
	UPhysicsAsset* PhysicsAsset = NTEBuildTool::FFModelPhysicsAssetImporter::ImportFromJsonFile(*SelectedSkeletalMesh, Selection.SourcePhysicsAssetJson, ImportOptions, Summary, Error);
	if (!PhysicsAsset)
	{
		NTEBuildTool::Editor::ShowError(FText::FromString(Error));
		return;
	...
	NTEBuildTool::Editor::ShowInfo(FText::Format(
		LOCTEXT("ImportedPhysicsAsset", "Imported {0} bodies, {1} constraints, and {2} disabled collision pairs into {3}. It is assigned to {4}. Save the dirty assets after checking it."),
...
void FNTEBuildToolModule::CreateModMaterialInstanceFromGameLibrary()
{
	RunCreateModMaterialInstanceFromGameLibrary(NTEBuildTool::Material::FNteMaterialInstanceOptions());
}
