...
bool ShowKawaiiImportOptionsDialog(const FString& SourceJsonPath, FString& OutTargetMeshId, FString& OutPresetPrefix)
{
	OutTargetMeshId = OutTargetMeshId.IsEmpty() ? TEXT("main") : OutTargetMeshId;
	...
	SAssignNew(Window, SWindow)
		.Title(LOCTEXT("KawaiiImportOptionsTitle", "Import Kawaii JSON"))
		.ClientSize(FVector2D(620, 190))
		...
		[
			...
			[
				SNew(STextBlock)
				.Text(FText::FromString(FString::Printf(TEXT("Source: %s"), *SourceJsonPath)))
			]
			+ SVerticalBox::Slot()
			.AutoHeight()
			.Padding(14, 0, 14, 8)
			[
				MakeEditableSpecRow(
					LOCTEXT("KawaiiImportTargetMeshId", "Target Mesh Id"),
					OutTargetMeshId,
					TargetMeshTextBox,
					[](const FString&) {})
			]
			...
			[
				...
				[
					...
					.OnClicked_Lambda([&]()
					{
						OutTargetMeshId = TargetMeshTextBox.IsValid() ? TargetMeshTextBox->GetText().ToString() : TEXT("main");
						OutPresetPrefix = PrefixTextBox.IsValid() ? PrefixTextBox->GetText().ToString() : FString();
						OutTargetMeshId.TrimStartAndEndInline();
						OutPresetPrefix.TrimStartAndEndInline();
						if (OutTargetMeshId.IsEmpty())
						{
							OutTargetMeshId = TEXT("main");
						}
						bAccepted = true;
						if (Window.IsValid())
						{
...
bool ImportKawaiiJsonIntoWorkspace(const TSharedRef<FWorkspaceDialogState>& State)
{
	ApplyTextBoxesToSpec(State);

	FString SourceJsonPath;
	if (!NTEBuildTool::Editor::ChooseJsonFileWithTitle(
		LOCTEXT("OpenKawaiiAnimLayerJson", "Open FModel Kawaii AnimBP/AnimLayer JSON"),
		TEXT("Physics_AnimLayer.json"),
		SourceJsonPath))
	{
		return false;
	}
	...
	FString PresetPrefix = FPaths::GetBaseFilename(SourceJsonPath);
	if (!ShowKawaiiImportOptionsDialog(SourceJsonPath, TargetMeshId, PresetPrefix))
	{
		return false;
	}
	...
	SAssignNew(Window, SWindow)
		...
		[
			...
			[
				...
				[
					SNew(STextBlock)
					.Text(LOCTEXT("WorkspaceKawaiiImportHint", "Import source-game Kawaii JSON as editable presets. The list shows target skeleton, missing bones, and tag diagnostics from KawaiiPlan."))
				]
				+ SHorizontalBox::Slot()
				.AutoWidth()
				.Padding(8, 0, 0, 0)
				[
					SNew(SButton)
					.Text(LOCTEXT("WorkspaceImportKawaiiJsonButton", "Import Kawaii JSON"))
					.OnClicked_Lambda([State]()
					{
						ImportKawaiiJsonIntoWorkspace(State);
						return FReply::Handled();
					})
				]
			]
