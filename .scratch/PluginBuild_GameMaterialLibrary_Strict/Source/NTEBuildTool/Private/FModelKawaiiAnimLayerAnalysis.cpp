...
FPlane GetPlaneField(const FJsonObject& Object, const TCHAR* FieldName)
{
	TSharedPtr<FJsonObject> PlaneObject;
	if (!TryGetObject(Object, FieldName, PlaneObject))
	{
		return FPlane(0, 0, 0, 0);
	}

	return FPlane(
		GetFloat(*PlaneObject, TEXT("X")),
		GetFloat(*PlaneObject, TEXT("Y")),
		GetFloat(*PlaneObject, TEXT("Z")),
		GetFloat(*PlaneObject, TEXT("W")));
}
...
FFModelKawaiiCollisionLimit AnalyzeLimit(const FJsonObject& LimitObject, const FString& LimitKind)
{
	...
	Result.Extent = GetVectorField(LimitObject, TEXT("Extent"));
	Result.Plane = GetPlaneField(LimitObject, TEXT("Plane"));
